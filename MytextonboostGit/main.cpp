...
const string fileFolder = "C:/Users/Zhiyu/Documents/Visual Studio 2013/Projects/MytextonboostGit/mri_data";
...
void toKmeans(vector<Mat> input, int k, vector<Mat> &output);
//class wavelet;
//class wavelet2;
...
void toKmeans(vector<Mat> input, int k, vector<Mat> &output)
{
	Mat centers, in, in_temp, ot,ot_temp;
	in = Mat::zeros(input[0].cols*input[0].rows, 1, CV_32F);
	imshow(input[0]);
	//Mat samples(in.rows * in.cols, 1, CV_32F);
	for (int i = 0; i<input[0].cols*input[0].rows; i++) 
	{
		in.at<float>(i, 0) = input[0].data[i];
	}
	/*for (int r = 0; r < input[0].rows; r += 10)
	{
		for (int c = 0; c < input[0].cols; c += 10)
		{
			cout << input[0].at<float>(r, c) << ",";
		}
		cout << endl;
	}*/
	in_temp = in.reshape(0, input[0].rows);
	imshow(in_temp);
	kmeans(in, k, ot, TermCriteria(CV_TERMCRIT_ITER, 100, 1.0), 1, KMEANS_PP_CENTERS, centers);
	normalize(ot, ot_temp, 0, 255, NORM_MINMAX, CV_32F);
	ot_temp = ot_temp.reshape(0, input[0].rows);
	//centers = centers.reshape(0, input[0].rows);
	for (int r = 0; r < ot_temp.rows; r += 10)
	{
		for (int c = 0; c < ot_temp.cols; c += 10)
		{
			cout << ot_temp.at<float>(r, c) << ",";
		}
		cout << endl;
	}
	/*for (int i = 0; i < 5; i++)
		cout << centers.at<float>(i, 0) << ",";*/
		
	imshow(ot_temp);
	output.push_back(ot.clone());
	printf("kmeans ends");
}
